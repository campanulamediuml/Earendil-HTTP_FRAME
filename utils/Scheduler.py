...
def IntervalTask_proc(sec, func, args=(), immediatly=True):
    sys = common.get_sys_info()
    print(sys)
    if sys != PLATFORM_TYPE_WINDOWS:
        def run(*args):
            if immediatly is False:
                time.sleep(sec)
            while 1:
        ...
        IntervalTask(sec, func, args, immediatly)
    # else:
...
def normal_task_proc(sec, func, args=()):
    sys = common.get_sys_info()
    if sys != PLATFORM_TYPE_WINDOWS:
        def run(*args):
            time.sleep(sec)
        ...
        normal_task(sec, func, args)


class task_pro(object):
    def __init__(self, sec, func, args=(), ident_name=''):
        self.res = None
        self.sec = sec
        self.func = func
        self.args = args
    ...
    def main(self):
        # print(self.args[0])
        t = multiprocessing.Process(target=self.method, args=self.args)
        t.start()
